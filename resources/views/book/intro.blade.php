...
Incidentally, you'll often see React referred to as React.js, because developers love tacking .js onto pretty much everything these days. However the official usage is indeed simply <em>React</em>.</p>
...
<li><strong>One-way Data Binding</strong>: React dramatically simplifies the process associated with ensuring the UI and current state of your application data by enforcing a rigorous flow of data from component owner to its children. You'll use a well-defined convention to listen for changes in &quot;state&quot; and informing the application to react (no pun intended) accordingly.</li>
<li><strong>Virtual DOM</strong>: React maintains a copy of the DOM in memory (known as the virtual DOM), and when informed of data-related changes, React uses this virtual representation to calculate the difference and then update only the necessary part of the DOM. This results in significant performance benefits.</li>
...
<li><strong>Testable</strong>: React applications are not monolithic; they are assembled from many relatively small, isolated components, which encourages automated testing. At the end of chapter 2 I'll show you how to test your React components.</li>
<li><strong>Open Source</strong>: React is released under a BSD license, and Facebook provides an additional patent grant. You can learn more about both the licensing and grant on the <a href="https://github.com/facebook/react">React README</a>.</li>
</ul>
...
<p>This book is organized into eight chapters, each of which is briefly described below.</p>
...
<p>If anything, basing much of the instruction around a thematic project keeps the material practical and helps to frame the discussion within the greater goal of creating a working application.</p>
